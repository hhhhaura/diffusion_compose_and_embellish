...
class MusicTokenizer(ABC, HFHubMixin):
    ...
    def _score_to_tokens(
        ...
        *TimeSignature*...) and track events (*Pitch*, *Velocity*, *Pedal*...) are
        ...
        # Zion: add chords from markers
        if self.config.use_chords:
            chords = []
            for marker in score.markers:
                if marker.text.startswith('global_bpm') or marker.text == 'N_N_N':
                    continue
                seg = marker.text.split('_')
                if len(seg) != 3:
                    continue
                root, chord, _ = seg
                assert root != 'Bb', marker.text
                if chord in self.config.chord_maps:
                    chords.append(
                        Event(
                            type_='Chord',
                            value=f'{root}:{chord}',
                            time=marker.time,
                            program=0,
                            desc=self.config.chord_maps[chord],
                        )
                    )
            all_events[0] += chords
