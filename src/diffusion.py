...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    if self.config.backbone == 'compose_and_embellish_dit': # yunchenc
      self.backbone = models.compose_and_embellish_dit.DIT(
  ...
  def forward(self, x, sigma, cond=None): # yuchenc
    """Returns log score."""
    sigma = self._process_sigma(sigma)

    logits = self.backbone(x, sigma, cond) # yunchenc
  ...
  def _compute_loss(self, batch, prefix):
    if 'attention_mask' in batch:
      ...
      attention_mask = None
    if self.config.melody_condition == True:
      losses = self._loss(batch['input_ids'], attention_mask, batch['cond_ids'])
    else:
  ...
  def on_validation_epoch_end(self):
    if ((self.config.eval.compute_perplexity_on_sanity
      ...
      for _ in range(
        ...
        # yunchenc conditioned generation
        if self.config.melody_condition == True: 
          samples = self._sample(cond=self.valid_dataset[_]['cond_ids'].unsqueeze(0))
        else: 
  ...
  def _ddpm_caching_update(self, x, t, dt, p_x0=None, cond=None): # yunchenc
    ...
    if p_x0 is None:
      p_x0 = self.forward(x, sigma_t, cond).exp() # yunchenc
  ...
  def _ddpm_update(self, x, t, dt, cond=None): # yunchenc
    sigma_t, _ = self.noise(t)
    ...
    log_p_x0 = self.forward(x, unet_conditioning, cond=cond)
  ...
  def _sample(self, num_steps=None, eps=1e-5, cond=None):
    ...
    for i in tqdm(range(num_steps)):
      ...
      if self.sampler == 'ddpm':
        x = self._ddpm_update(x, t, dt, cond=cond) # yunchenc
      elif self.sampler == 'ddpm_cache':
        p_x0_cache, x_next = self._ddpm_caching_update(
          x, t, dt, p_x0=p_x0_cache, cond=cond) # yunchenc
    ...
    if self.config.sampling.noise_removal:
      ...
      if self.sampler == 'analytic':
        ...
        x = self.forward(x, unet_conditioning, cond=cond).argmax(dim=-1) # yunchenc
    return x

  def restore_model_and_sample(self, num_steps, eps=1e-5, cond=None): # yunchenc
    """Generate samples from the model."""
    # Lightning auto-casting is not working in this method for some reason
    if self.ema:
      self.ema.store(itertools.chain(
    ...
    self.backbone.eval()
    self.noise.eval()
    samples = self._sample(num_steps=num_steps, eps=eps, cond=cond) # yunchenc
    if self.ema:
  ...
  def _maybe_sub_sample(self, x0, attention_mask, cond=None):
    seqlen = x0.shape[1]
  ...
  def _reconstruction_loss(self, x0, cond=None): # yunchenc
    t0 = torch.zeros(x0.shape[0], dtype=self.dtype,
                     device=self.device)
    assert self.config.noise.type == 'loglinear'
    # The above assert is for d3pm parameterization
    unet_conditioning = self.noise(t0)[0][:, None]
    model_output_t0 = self.forward(x0, unet_conditioning, cond=cond) # yunchenc
    return - torch.gather(input=model_output_t0,
                          dim=-1,
                          index=x0[:, :, None]).squeeze(-1)

  def _forward_pass_diffusion(self, x0, cond=None): # yunchenc
    ...
    model_output = self.forward(xt, unet_conditioning, cond)
    ...
    if self.T > 0:
      ...
      if self.parameterization == 'd3pm':
        reconstruction_loss = self._reconstruction_loss(x0, cond=cond) # yunchenc
  ...
  def _loss(self, x0, attention_mask, cond=None): # yunchenc
    ...
    if self.parameterization == 'ar':
      ...
      loss = self._forward_pass_diffusion(input_tokens, cond=cond) # yunchenc
